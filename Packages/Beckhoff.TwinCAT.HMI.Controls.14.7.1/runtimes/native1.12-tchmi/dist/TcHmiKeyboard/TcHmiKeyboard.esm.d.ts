import { Controls, EventProvider, Locale, ObjectResolver, TcHmiControl, type Color, type DestroyFunction, type Dictionary, type DimensionUnit, type FontFamily, type FontSizeUnit, type FontStyle, type FontWeight, type FourSidedCss, type HorizontalAlignment, type PixelFourSidedCss, type ScaleModeString, type SelectableRequired, type SizeModeWithContent, type SolidColor } from 'Beckhoff.TwinCAT.HMI.Framework/index.esm.js';
...
declare class TcHmiKeyboard extends TcHmiControl.Control {
    ...
    /**
     * Internal reference to the attribute "data-tchmi-layout-file"
     */
    protected __layoutFile: string | null | undefined;
    /**
     * Internal reference to the attribute "data-tchmi-current-layout"
     */
    protected __currentLayoutAttribute: string | null | undefined;
    /**
    ...
    protected __compositionKey: SelectableRequired<StandardKey, 'composition'> | null;
    ...
    /**
     * Clears an ongoing interaction with a key. Stops the longpress/repeat timer of the key, removes it
     * from the interacted keys and clears its visual state, so a repeat key (for example backspace) can
     * never keep auto-repeating after the interaction has ended. Does not dispatch a key release, which
     * matches the behavior of leaving a key while it is still pressed. The caller is responsible for
     * raising the onKeyPressFinished or onKeyPressCanceled event.
     * @param keyElement The key HTML element whose interaction should be cleared.
     * @returns Whether an interaction for the given key element was found and cleared.
     */
    protected __clearKeyInteraction(keyElement: HTMLElement): boolean;
    /**
     * Clears all ongoing key interactions and stops their longpress/repeat timers. Used as a safety net
     * when the keyboard is detached or hidden so that no timer can outlive the keyboard and keep sending
     * key presses (for example backspace) to a later focused input. Like __clearKeyInteraction this only
     * tears down the interaction state and deliberately does not raise the onKeyPressFinished or
     * onKeyPressCanceled event, so it stays consistent with the single key clear.
     */
    protected __clearAllKeyInteractions(): void;
    /**
    ...
    /**
     * Gets the value of the 'data-tchmi-available-layouts' readonly attribute.
     */
    getAvailableLayouts(): string[];
    /**
     * Sets the value of the 'data-tchmi-current-layout' attribute.
     * @param valueNew The new value for currentLayout.
     */
    setCurrentLayout(valueNew: string | null): void;
    /**
     * Sets the value of the 'data-tchmi-current-layout' attribute.
     * @param valueNew The new value for currentLayout.
     */
    protected __setCurrentLayout(valueNew: string | null, process?: boolean): void;
    /**
     * Gets the value of the 'data-tchmi-current-layout' attribute.
     */
    getCurrentLayout(): string | null | undefined;
    /**
     * Processes the value of the 'data-tchmi-current-layout' attribute.
     */
    protected __processCurrentLayout(): void;
    /**
     ...
     */
    protected __onResolverForIndirectInputPaddingWatchCallback(data: ObjectResolver.IWatchResultObject<FourSidedCss | null>): void;
    /**
     ...
     * The watch callback for the keyboardPadding object resolver.
     */
    protected __onResolverForKeyboardPaddingWatchCallback(data: ObjectResolver.IWatchResultObject<PixelFourSidedCss>): void;
    /**
     ...
     */
    protected __onResolverForKeyPaddingWatchCallback(data: ObjectResolver.IWatchResultObject<PixelFourSidedCss>): void;
    /**
     ...
     */
    protected __onResolverForAdditionalKeyBackdropPaddingWatchCallback(data: ObjectResolver.IWatchResultObject<FourSidedCss>): void;
    /**
     ...
     */
    protected __onResolverForKeyBackgroundColorWatchCallback(data: ObjectResolver.IWatchResultObject<Color>): void;
    /**
     ...
     */
    protected __onResolverForLabelColorWatchCallback(data: ObjectResolver.IWatchResultObject<SolidColor>): void;
    /**
     ...
     */
    protected __onResolverForLabelStrokeColorWatchCallback(data: ObjectResolver.IWatchResultObject<SolidColor>): void;
    /**
     ...
     */
    protected __onResolverForLabelFillColorWatchCallback(data: ObjectResolver.IWatchResultObject<Color>): void;
    /**
     ...
     */
    protected __onResolverForAdditionalKeyBackdropColorWatchCallback(data: ObjectResolver.IWatchResultObject<Color>): void;
    /**
